    ...
    if (movingAverageHistory.length > HistorySize) {
      ...
      if (movingAverage > previousMovingAverage * BuyThreshold) {
        ...
        if (cash > stockPrice + COMMISION_FEE) {
          ...
          newShares = Math.floor (
            (cash - additionalCost - COMMISION_FEE) / stockPrice
          );
          while (!buyStock (StockSymbol, newShares) && newShares > 0) {
            ...
            newShares = Math.floor (
              (cash - additionalCost - COMMISION_FEE) /
          ...
          if (newShares > 0) {
            ...
            if (prevStockData[0] === 0) {
              ...
              cost =
                COMMISION_FEE +
                (stockData[1] * stockData[0] -
                  prevStockData[1] * prevStockData[0]);
            }
            cost += additionalCost;
            cash -= cost;
